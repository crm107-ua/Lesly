...
$.fn.calendar = function(options) {
    ...
    this.each(function() {
        ...
        var calendar_id =
            "cal_" + Math.floor(Math.random() * 99999).toString(36);

        var events = args.events;

        var days;
        if (typeof args.days != "undefined") days = args.days;
        else
            days = [
                "Domingo",
                "Lunes",
        ...
        var months;
        if (typeof args.months != "undefined") months = args.months;
        else
            months = [
                "Enero",
                "Febrero",
        ...
        var show_days;
        if (typeof args.show_days != "undefined") show_days = args.show_days;
        else show_days = true;

        var popover_options;
        if (args.popover_options != "undefined")
            popover_options = args.popover_options;
        else popover_options = { placement: "top" };

        var tooltip_options;
        if (typeof args.tooltip_options != "undefined")
            tooltip_options = args.tooltip_options;
        else tooltip_options = { placement: "top" };

        var req_ajax;
        if (typeof args.req_ajax != "undefined") req_ajax = args.req_ajax;
        else req_ajax = false;
        ...
        function showCalendar() {
            ...
            calendar = $(
                '<div class="calendar" id="' + calendar_id + '" ></div>'
            );
        ...
        function markEvents(month, year) {
            ...
            for (var i = 0; i < events.length; i++) {
                if (
                    events[i][0].split("/")[1] == t_month &&
                    events[i][0].split("/")[2] == year
                ) {
                    $(
                        "#" +
                            calendar_id +
                            "_" +
                            events[i][0].replace(/\//g, "_")
                    ).addClass("event");
                    ...
                    $(
                        "#" +
                            ...
                            events[i][0].replace(/\//g, "_") +
                            " a"
                    ).attr("data-original-title", events[i][1]);
            ...
            $("#" + calendar_id + " " + ".event_tooltip a").tooltip(
                tooltip_options
            );
            $("#" + calendar_id + " " + ".event_popover a").popover(
                popover_options
            );
